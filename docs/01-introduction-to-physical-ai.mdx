...
Yet despite these breakthroughs, AI remained **disembodied**—confined to screens, servers, and virtual environments.
...
<details>
<summary>🪟 **Windows + WSL2 Quick-Start for This Chapter**</summary>
...
**Example**: When a humanoid steps on uneven ground, the IMU detects tilt (10ms), MPC adjusts torso angle (20ms), motors compensate (30ms)—all faster than human reaction time (200ms).
...
**Example**: NVIDIA Isaac ROS accelerates VSLAM (Visual SLAM) on Jetson Orin to less than 50ms latency using GPU-optimized algorithms.
...
Training embodied AI requires **demonstration data**—examples of tasks being performed in the real world.
...
Example: Train a humanoid to fold laundry by watching 10,000 YouTube videos of humans folding clothes → Vision-Language-Action model (RT-2) maps video → motor commands.
...
- **Hardware**: Intel RealSense D435i (640×480 @ 90fps), Logitech C920 (1080p @ 30fps)
...
:::info For Beginners
This code uses the `pyrealsense2` library to connect to an Intel RealSense camera and read depth data.
:::
...
**Purpose**: High-precision 3D mapping (±2cm accuracy)

**How it works**:
1. Emit laser pulses (905nm or 1550nm wavelength)
2. Measure time-of-flight: Distance = (Speed of Light × Time) / 2
3. Rotate laser (360°) to scan entire environment
...
for scan in lidar.iter_scans():
    ...
    print(f"Closest obstacle: {distances[min_dist_idx]:.0f}mm at {angles[min_dist_idx]:.1f}°")
...
**Expected Output**:
```
Closest obstacle: 450mm at 32.5°
```
...
**Components**:
- **Accelerometer**: Measures linear acceleration (x, y, z axes) in m/s²
...
**How it works**: Strain gauges embedded in robot joints measure force (N) and torque (N⋅m)

**Hardware**:
- **Budget**: FSR (Force-Sensitive Resistor) pads ($5, binary touch detection)
- **Mid-range**: ATI Mini40 ($2,500, 6-axis F/T sensor, ±40N, ±2N⋅m)
- **Premium**: ATI Gamma ($8,000, ±65N, ±5N⋅m, used in da Vinci surgical robot)
...
**Example**: A grasp controller trained in Isaac Sim applies exactly 10.0N. In reality, motor backlash means actual force varies between 8.5N - 11.5N → object slips or crushes.
...
- Camera noise (Gaussian σ=5 pixels)
- Lighting (brightness, shadows, reflections)
- Motor delays (0-50ms)

**Result**: Policy learns to handle variability → transfers better to real world.

**NVIDIA Isaac Sim** has built-in domain randomization tools (Chapter 6).

:::tip Advanced
**Research frontier**: Use real-world data to fine-tune sim-trained policies (Sim2Real2Sim). Collect 1 hour of real robot data → update simulator physics → retrain policy → deploy. Iterative loop closes sim-to-real gap (Tan et al., ICRA 2018).
:::
...
**Scenario**: Robot scans environment with LIDAR → detects obstacles → turns away from closest obstacle.
...
class SimpleRobot:
    ...
    def sense_lidar(self, obstacles):
        ...
        angles = np.linspace(0, 2*np.pi, 360)  # 360° scan
        distances = []
    ...
    def avoid_obstacle(self, angles, distances):
        ...
        if distances[min_idx] < 1.0:  # Obstacle within 1 meter
            ...
            print(f"⚠️ Obstacle at {distances[min_idx]:.2f}m, turning {np.degrees(turn_angle):.0f}°")
        else:
...
for step in range(10):
    print(f"\n--- Step {step} ---")
    print(f"Position: {robot.position}, Heading: {np.degrees(robot.heading):.1f}°")
...
--- Step 1 ---
Position: [0.5 0. ], Heading: 0.0°
✅ Path clear, moving forward

--- Step 2 ---
Position: [1. 0.], Heading: 0.0°
⚠️ Obstacle at 0.95m, turning 178°
...
```
...
This is a simplified version of **reactive navigation**—robots that respond to immediate sensor input without planning.
:::
...
3. Compare to simulated camera noise (Gaussian σ=5 pixels)
...
**Next Chapter**: [Chapter 2: ROS 2 Fundamentals →](02-ros2-fundamentals.mdx)
